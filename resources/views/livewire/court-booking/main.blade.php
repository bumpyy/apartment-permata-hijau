...
<div>
    <!-- Header Component -->
    @include('livewire.court-booking.ui.header')

    <div class="container mx-auto min-h-screen bg-white px-4 py-6">
        <!-- Navigation Component -->
        @include('livewire.court-booking.ui.navigation')

        <!-- Booking Rules Component -->
        @include('livewire.court-booking.ui.booking-rules')

        <!-- Login Prompt Component -->
        @include('livewire.court-booking.ui.login-prompt')

        <!-- Quota Display Component -->
        @include('livewire.court-booking.ui.quota-display')

        <!-- Quota Warning Component -->
        @include('livewire.court-booking.ui.quota-warning')

        <!-- Real-time Status Component -->
        @include('livewire.court-booking.ui.real-time-status')



        <!-- Weekly View -->
        @if ($viewMode === 'weekly')
            @include('livewire.court-booking.views.weekly-view')
        @endif

        <!-- Monthly View -->
        @if ($viewMode === 'monthly')
            @include('livewire.court-booking.views.monthly-view')
        @endif

        <!-- Daily View -->
        @if ($viewMode === 'daily')
            @include('livewire.court-booking.views.daily-view')
        @endif


        <!-- Selection Summary Component -->
        @include('livewire.court-booking.ui.selection-summary')

        <!-- Legend Component -->
        @include('livewire.court-booking.ui.legend')

        <!-- Confirm Button Component -->
        @include('livewire.court-booking.ui.confirm-button')
    </div>

    <!-- Time Selector Modal -->
    @include('livewire.court-booking.modals.time-selector')

    <!-- Enhanced Date Picker Modal -->
    @include('livewire.court-booking.modals.date-picker')

    <!-- Confirmation Modal -->
    @include('livewire.court-booking.modals.confirmation')

    <!-- Thank You Modal -->
    @include('livewire.court-booking.modals.thank-you')
