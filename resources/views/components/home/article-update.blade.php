@props(['variant' => 'basic', 'title' => '', 'data' => [], 'columnNumber' => count($data)])
    ...
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div class="sm:grid-cols-(--custom-columns) mx-auto mt-16 grid max-w-2xl auto-rows-fr grid-cols-1 gap-8 sm:mt-20 lg:mx-0 lg:max-w-none"
            ...
            @foreach ($data as $item)
                <x-dynamic-component :component="'home.news-update.cards.' . $variant" date="{{ date('F j, Y') }}" :image="'https://picsum.photos/seed/' . $loop->index . '/640/480'" :title="$item['title']" />
            @endforeach
        </div>
    </div>
</section>
